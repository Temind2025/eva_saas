<div style="width: 25%;">
  @if(count($data->services) > 1 ||  $data->bookedPackageService->count() > 1)
    <a class="badge bg-info text-white" href="javascript:void(0)" data-bs-toggle="modal" data-bs-target="#service-detail-modal-{{ $data->id }}">{{ __('messages.multiple') }}</a>
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header">
            <h1 class="modal-title fs-5" id="service-detail-modal-{{ $data->id }}-Label">{{ __('messages.service_detail') }}</h1>
