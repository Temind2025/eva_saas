                ...
                <div class="row align-items-center">
                    ...
                    <div class="form-group col-md-6">
                        ...
                        <div class="form-group form-check form-switch form-control d-flex align-items-center justify-content-between gap-3">
                            ...
                            <input class="form-check-input" name="status" type="checkbox" id="statusSwitch" onchange="toggleStatusLabel()" {{ (isset($user) && $user->status == 0) ? '-' : 'checked' }}>
                        </div>
                    </div>
                </div>
...
$('#user-form').on('submit', function(e) {
    ...
    $(document).ready(function () {
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response.exists) {
                        ...
                        if (field === 'mobile') {
                            $('#mobile-error').text(response.message).show();
                        } else {
                            $('#' + field).next('.error').text(response.message).show();
                        }
                    } else {
        ...
        $('#email').blur(function () {
            ...
            if (email) checkField('email', email);
        });

        $('#mobile').blur(function () {
            const mobile = $('#mobile').val();
            if (mobile) checkField('mobile', mobile);
        });
    });
    ...
    function toggleStatusLabel() {
        ...
        if (statusSwitch.checked) {
            statusLabel.innerText = '{{ __("messages.active") }}';
        } else {
            statusLabel.innerText = '{{ __("messages.inactive") }}';
        }
    }
